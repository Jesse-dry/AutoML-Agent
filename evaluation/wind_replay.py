# Wind Task 1–15 × Zone 1–10 滚动回放主循环
# ---------------------------------------------------------------
# 逐 Task → 逐 Zone：build_wind_task → 严格泄漏检查（中止违规）→ fit 独立后端
# → 滚动预测 → 指标。Task 得分 = 10 分区指标均值（逐分区独立模型）。
#
# 与 Load 版（evaluation/task_replay.py）的差异：
#   - 多 Zone 维度：每 Task 循环 10 分区，各自独立建模
#   - 预测窗口特征含气象外生列（TaskExpVars 预报，决策时点可得）
#   - 目标为 TARGETVAR（归一化风电出力 [0,1]）
...
from data.wind_loader import WIND_DATA_DIR, WIND_ZONES
...
def _raise_leakage(where: str, violations) -> None:
    ...
    raise LeakageError(f"[LEAK] Wind {where} 泄漏检查未通过:\n{msgs}")
...
def wind_rolling_predict(
    ...
    气象外生列 = 历史气象 ∪ 预测月预报（TaskExpVars），决策时点可得，不随协议回填。
    """
    history_target = task.history_df[task.target_col]
    weather_full = pd.concat(
        [task.history_df[WIND_WEATHER_DERIVED_COLS],
         task.weather_forecast_df[WIND_WEATHER_DERIVED_COLS]]
    )

    if not protocol.recursive:
        observed = pd.concat([history_target, task.y_true])
        X = build_wind_forecast_features(observed, weather_full, task.forecast_ts, spec)
    ...
    for t in task.forecast_ts:
        x = _wind_features_at(observed, weather_full, t, spec, task.target_col)
...
def _check_wind_task_leakage(task, leak_check: str, protocol: ForecastProtocol,
                             spec: List[dict]) -> None:
    """对历史段（+ 预测窗口，非 recursive 时）做严格泄漏检查，违规即中止。

    与 Load 版同构：Pass A 血缘静态检查 + Pass B recompute。气象外生列
    （ws10/ws100 等）为 spec 的非目标 source，Pass A 只校验血缘元数据；
    ...
    if not protocol.recursive:
        ...
        fw_feat = build_wind_forecast_features(observed_full, weather_full,
...
def summarize_wind(zone_results: Dict[int, List[TaskResult]]) -> Dict[str, object]:
    """
    汇总 Wind 回放结果：
      detail_table — 逐 Task×Zone 一行指标
      task_table   — 每 Task 一行 = 10 分区指标均值（zone="MEAN"）
...
def replay_wind(
    ...
    """
    Wind 滚动回放主循环。返回 {zone_results, detail_table, task_table, summary}。
    ...
    for tid in sorted(task_ids):
        ...
        for zone in zones:
            task = build_wind_task(tid, zone, data_dir, val_hours, spec)
            if not skip_leak_check:
                _check_wind_task_leakage(task, leak_check, protocol, spec)
            backend.fit(task.train_df, task.val_df, task.feature_cols,
                        task.target_col, seed)
            y_pred = wind_rolling_predict(backend, task, protocol, spec)
    ...
    summary_dict = summarize_wind(zone_results)
    payload = {
