...
def build_task(
    ...
    """
    构建 Task：可用历史 → 整段一次性特征 → 切 train/val → 取预测月真值。
    ...
    hist = feat.copy()
    hist[TARGET_COL] = av.history_df[TARGET_COL].values

    if len(hist) <= MAX_LAG + val_hours:
        raise ValueError(
            f"Task {task_id} 历史过短（{len(hist)} 行），无法满足 "
