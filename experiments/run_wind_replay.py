# GEFCom2014-W Task 1–15 × Zone 1–10 无泄漏滚动回放评测 CLI
# ---------------------------------------------------------------
# 用法示例：
#   python experiments/run_wind_replay.py --tasks 1:15 --model lightgbm
#   python experiments/run_wind_replay.py --tasks 1:3  --model persistence
#   python experiments/run_wind_replay.py --tasks 1:15 --model lightgbm --zones 1:3 --outdir experiments/output/wind_replay
#
# 输出：逐 Task（10 分区均值）指标表 + 逐 Task×Zone 明细；--outdir 指定审计输出。
...
from evaluation.wind_replay import LeakageError, replay_wind  # noqa: E402
...
def parse_zones(s: str):
    """解析 '1:10' / '1,3,5' / '1-5' 为 Zone 列表（1..10）。"""
    s = s.strip()
    out = []
    for part in s.split(","):
        part = part.strip()
    ...
    if any(z < 1 or z > 10 for z in zones):
        raise ValueError("zone 必须在 1..10")
    return zones
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="GEFCom2014-W Task 1-15 x Zone 1-10 无泄漏滚动回放评测")
    parser.add_argument("--tasks", default="1:15",
                        help="任务范围，如 1:15 / 1,3,5 / 1-5 / 1:15:2")
    parser.add_argument("--zones", default="1:10",
                        help="分区范围，如 1:10 / 1,3,5 / 1-5")
    ...
    parser.add_argument("--outdir", default=None,
                        help="审计输出目录（默认 experiments/output/wind_replay）")
    ...
    try:
        ...
        outdir = Path(args.outdir) if args.outdir else (
            PROJECT_ROOT / "experiments" / "output" / "wind_replay"
        )
        ...
        if args.model == "seasonal_naive_all":
            ...
            for k in (24, 168):
                ...
                print(f"\n--- {backend.name} ---")
                payload = replay_wind(tasks, backend, protocol,
