# GEFCom2014-P Task 1–15 无泄漏滚动回放评测 CLI
# ---------------------------------------------------------------
# 用法示例：
#   python experiments/run_price_replay.py --tasks 1:15 --model lightgbm
#   python experiments/run_price_replay.py --tasks 1:3  --model persistence
#   python experiments/run_price_replay.py --tasks 1:15 --model lightgbm --outdir experiments/output/price_replay
#
# 输出：逐 Task（预测日 24h）指标表；--outdir 指定审计输出。
...
from evaluation.price_replay import LeakageError, replay_price  # noqa: E402
...
def _print_table(summary_dict) -> None:
    ...
    table["MAPE"] = table["MAPE"].map(lambda v: f"{v:.2f}%" if v is not None else "-")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="GEFCom2014-P Task 1-15 无泄漏滚动回放评测")
    ...
    parser.add_argument("--outdir", default=None,
                        help="审计输出目录（默认 experiments/output/price_replay）")
    ...
    try:
        ...
        outdir = Path(args.outdir) if args.outdir else (
            PROJECT_ROOT / "experiments" / "output" / "price_replay"
        )
        ...
        if args.model == "seasonal_naive_all":
            ...
            for k in (24, 168):
                ...
                print(f"\n--- {backend.name} ---")
                payload = replay_price(tasks, backend, protocol,
        ...
        print(f"模型: {backend.name}  协议: {protocol.name}  Tasks: {tasks}  "
              f"seed={args.seed}  val_hours={args.val_hours}  "
